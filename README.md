# Cyber-Learning.fr

Bienvenue sur le dépôt GitHub du site [Cyber-Learning.fr](https://cyber-learning.fr) !

Cyber-Learning.fr est une plateforme éducative dédiée à l'apprentissage en ligne, spécialement conçue pour les étudiants et enseignants en informatique. Elle propose des ressources, des exercices interactifs et des outils pour améliorer les compétences en développement web, bases de données, et bien plus encore.

---

## 🚀 Fonctionnalités

- **TP/TD interactifs** : Des contenus pédagogiques clairs et organisés par niveaux et thématiques.
- **Exercices pratiques** : Des exercices pour s'entraîner et approfondir ses connaissances en cyber sécurité.
- **Suivi de progression** : Des tableaux de bord pour suivre les compétences et identifier les axes d'amélioration.

---

## Solutions

+ 13 rotations ?	
+ 13375P34K	
+ 5'->3	
+ 8000 tours !	
+ A la main...	
+ Alice et Bob	
+ Alice, Bob, Ronald, Adi et Leonard	
+ Au bout du rouleau	
+ Audioactive	
+ Base 64	
+ Bionicle	
+ Blue Monday	
+ C'est la rose je croiX	
+ Call me Baby	
+ Ceci n'est pas un arbre	
+ Cerveau en vrille	
+ Chiffrement à dents de scie	
+ Cléopatre	
+ Cléopatre le retour	
+ codage A1Z26	
+ Code bien barré	
+ Crypto Art	
+ Da Friderici Code	
+ Des chiffres et des lettres	
+ Dick... aurait aimé celle là.	
+ Digicode	
+ Enigma	
+ esianoloP	
+ Faction Collatz	
+ Fréquence Solaire	
+ Gravure d'alliance	
+ Invasion of the Vorticons	
+ IV and Let Die	
+ Joyeux anniversaire	
+ Knowledge Is Power	
+ L'évadé de Colchester	
+ La clé du carré	
+ Le carré de Wheatstone	
+ Le grand chiffre de louis xiv	
+ Le Manuscrit de Fuxi	
+ Le matin de trop	
+ Le shérif de l'espace	
+ Les danseurs de Conan	
+ Les signes du Zodiac	
+ Les Six Lettres du Front	
+ Lieu de résistance	
+ Lockpicking	
+ Mais c'est Ook !!!!!!	
+ Mais qu'est ce que tu dis Kenny ?	
+ Messages d'Evpatoria	
+ Mieux que le binaire ? le Unaire !	
+ NakNAk ?	
+ No mod, No sec	
+ Numéro de téléphone ?	
+ Palette de couleur	
+ Pas de malaise Blaise	
+ Pixellisation	
+ Point par point	
+ Premiers indices	
+ Presque...	
+ Random Is Not Random	
+ Retour à 2 facteurs	
+ Sacré Jean-Seb	
+ Sacré Saknussemm	
+ Scie à grosses dents	
+ sha256	
+ Slumcode Millionaire	
+ Sms en mode Intuitif	
+ Tactile	
+ Tiens la porte Walder !	
+ Transmission sans fil	
+ Transposition de Colonnes	
+ Trop fort les indiens	
+ TwinHex	
+ Un Polybe sans clé	
+ Unix to Unix	
+ We will, we will rockyou !	
+ We will, we will rockyou II	
+ Xor du BMP	
+ Xor du PNG	
+ 0x80780119	
+ 7 différences ?	
+ A la baguette !	
+ Angle de vue	
+ Around The World	
+ Attention les yeux	
+ Back In Black	
+ Ca n'est pas le chat d'Arnold	
+ Cinéma Tchi Tcha !	
+ code rouge sur l'ECG	
+ Composition avec grand plan rouge	
+ Crêpe de pixels	
+ Faut Arrêter les Questions !?	
+ Impression couleur	
+ Le pirate de l'espace	
+ Légende	
+ Liste de courses	
+ Mon beau sapin	
+ Morton ne répond plus	
+ Noir c'est noir !	
+ Photomaton	
+ Pixel Drift	
+ Projet X	
+ Quelques polygones 	
+ Silence entre les mots	
+ Smack Ohh Smack	
+ Stich est accro	
+ Suivez la ligne	
+ Un ptit peu de bleu en moins	
+ Une bien jolie lettre	
+ Voix d'un spectre	
+ Windows 1685	
+ 1+1=10	
+ 85 nuances pour un PDF	
+ Base 16	
+ Base 5 ? oui base 5 !	
+ Base 65	
+ Base Totof	
+ Base tournante	
+ Base Uranium	
+ Boby comptait en bibi !	
+ C'est signé !	
+ Des ptits trous...	
+ Diagonale d'un carré ????	
+ Disjonction inclusive	
+ EAD+GB+E	
+ Et ca fait quoi ?	
+ Fréquence clandestine	
+ GaBu ?	
+ La Mantisse Prend la Float	
+ Le monde se divise en 10 catégories	
+ Live Long and Decrypt	
+ Métropolis	
+ Mieux que Zooko	
+ Missing Element	
+ Octal 	
+ Patchwork	
+ Pavé de lettres (new version)	
+ Petits bâtons lumineux	
+ Reverse Hexadecimal	
+ Télégraphe	
+ Tu comptes comment Tao ?	
+ Voyage voyage !	
+ Appel à un ami ?	
+ C'est cadeau ! (CSRF)	
+ Cache Poisoning	
+ Canal privé 	
+ Click Jacking	
+ Cookie 2 : le retour.	
+ Cookies	
+ DOM Based XSS	
+ Faille classique de l'admin	
+ HTML - Javascript Obfuscation 1	
+ Interception	
+ Intermédiaire	
+ Javascript Obfuscation 2	
+ le bak de l'IDE	
+ Login coté client	
+ Mise à l'index	
+ Navigateur Mozzarella only	
+ Parcoursup Suite	
+ Passage passable	
+ Pixel WAR ONE	
+ Pour 450 points de plus ! (CSRF)	
+ Premier test de faille ?	
+ Prompt Injection	
+ Remote Cache Poisoning	
+ Retour à la source	
+ Server-side request forgery (SSRF)
+ Sondage	
+ SQL - Blind Injection	
+ SQL - Injection	
+ SQL - Injection 2	
+ SQL - Injection 2 - Filtrée	
+ SQL - Injection Time Based	
+ SQL - L'inscription de l'espace	
+ SQL - ON DUPLICATE	
+ SQL - Routed Injection	
+ Stored XSS 	
+ Trust No Link	
+ Un style de contournement	
+ Une question de méthode	
+ Xss Dump Page	
+ Xss Eval	
+ XSS Reflected	
+ Base 64 décodage	
+ Blockchain CL	
+ Calcul mental	
+ Clé partagée	
+ Code-Barres	
+ Deux fois de suite ?	
+ Etes vous un robot ?	
+ Labyrinthe	
+ Mastermind	
+ Parcoursup	
+ Pousse-Pousse	
+ Questions pour un champion	
+ Quick Response déCode	
+ Request	
+ Capture corrompue	

+ Cisco	
+ Curling	
+ Don't worry, be API	
+ Exfiltration Fantôme	
+ Exfiltration réseau	
+ Fantôme sur le Réseau	
+ ID Enocean	
+ IMP-IMP	
+ Infiltration du bottin	
+ Interception Jabber	
+ IP destination	
+ IP source	
+ L'IA c'est baud	
+ La Fuite de Roasting	
+ MAC destination	
+ MAC source	
+ Masque réseau	
+ Masque réseau II	
+ One Key to Rule Them All	
+ Opération SMS	
+ Payload Enocean	
+ Personal System / 2	
+ Ping Forensics	
+ Ping Forensics 2	
+ Ping Forensics 3	
+ SCADA Wire Dump	
+ Série abandonnée	
+ Signal fantôme sur le WIFI	
+ Slow Scan Television	
+ Stockage Interplanétaire	
+ Telnet (new version)	
+ Tempête de broadcast	
+ Trame incomplète	
+ Transpac	
+ Vieille impression	
+ BTSnooP	
+ BufferOverFLow	
+ C'est clair !	
+ C'est moins clair !	
+ Crack PDF 	
+ CryptXor	
+ ELF Unlocked	
+ Email douteux	
+ IDA est votre amie	
+ Logs	
+ Ransomware	
+ Winsock32.dll	
+ CLU-2 vs MCP	
+ CyberBoy	
+ CyberNES	
+ Dosage critique	
+ ESPER ENHANCEMENT	
+ HAL 9000	
+ MU/TH/UR	
+ Raffinage  de chiffres	

